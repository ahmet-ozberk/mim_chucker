...
/// Chopper interceptor for MimChucker
class MimChopperInterceptor {
  ...
  // Isolates are supported on desktop platforms (Windows, Linux, macOS)
  ...
  Future<Request> onRequest(Request request) async {
    if (!MimChuckerUtils.shouldInterceptRequest()) {
      return request;
    }
    ...
    final apiName = MimChuckerUtils.getApiNameFromRequest(request);
    final searchKeywords =
        MimChuckerUtils.getSearchKeywordsFromRequest(request);
  ...
  Future<Response<dynamic>> onResponse(Response<dynamic> response) async {
    ...
    final requestTime = _requestTimeMap[requestId] ??
  ...
  Future<void> _processResponseInIsolate(
    ...
    try {
      ...
      final apiResponse = _processChopperResponseMainThread(
          response, requestTime, apiName, searchKeywords);
      unawaited(_databaseService.addApiResponse(apiResponse));
      if (showNotification) {
        _notificationService.showNotification(apiResponse,
            maxBodySize: maxNotificationBodySize);
...
class MimChuckerHttpLoggingInterceptor {
  Future<Request> onRequest(Request request) async {
    if (!MimChuckerUtils.shouldInterceptRequest()) {
      return request;
    }
