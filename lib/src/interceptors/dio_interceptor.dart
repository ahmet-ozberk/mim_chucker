...
class MimDioInterceptor extends Interceptor {
  ...
  void onRequest(
    ...
    options.extra['mim_chucker_request_time'] = DateTime.now();
    return handler.next(options);
  }
  ...
  static void _processResponseIsolate(_ResponseData data) {
    ...
    try {
      final requestTime = response
              .requestOptions.extra['mim_chucker_request_time'] as DateTime? ??
  ...
  static void _processErrorIsolate(_ErrorData data) {
    ...
    try {
      ...
      final requestTime =
          error.requestOptions.extra['mim_chucker_request_time'] as DateTime? ??
              DateTime.now().subtract(const Duration(milliseconds: 100));
