...
class DatabaseService {
  ...
  static const String _databaseName = 'mim_chucker.db';
  ...
  Future<Database> _initDatabase() async {
    ...
    if (kIsWeb) {
      ...
      // Use in-memory database or throw informative error
      throw UnsupportedError(
        'MimChucker database is not supported on web platform. '
        'Use MimChucker only in debug mode or disable database storage.',
      );
    }
  ...
  static Future<void> _insertApiResponseIsolate(
    ...
    try {
      // Initialize FFI in isolate if needed
      if (!kIsWeb && (Platform.isWindows || Platform.isLinux || Platform.isMacOS)) {
        sqfliteFfiInit();
        sqflite.databaseFactory = databaseFactoryFfi;
      }
